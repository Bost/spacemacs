...
To install it, just add =themes-megapack= to your =~/.spguimacs= like so:
...
practice to store these layers in there, along with your =~/.spguimacs= file.
...
=dotspacemacs-configuration-layer-path= of your =~/.spguimacs=.
...
User configuration can be stored in your =~/.spguimacs= file.

** Dotfile Installation
The very first time Spacemacs starts up, it will ask you several questions
and then install the =.spguimacs= file in your =HOME= directory.

** Alternative dotdirectory
A dotdirectory =~/.spguimacs.d/= can be used instead of a dotfile.
If you want to use this option, move =~/.spguimacs= to =~/.spguimacs.d/init.el=.

It is also possible to override the location of =~/.spguimacs.d/= using the
environment variable =SPACEMACS_GUIX_DIR=. Of course, you can also use symlinks to
change the location of this directory.

*Note*: =~/.spguimacs= will always take priority over =~/.spguimacs.d/init.el=,
so =~/.spguimacs= must not exist for =~/.spguimacs.d/init.el= to be used by
Spacemacs.

** Synchronization of dotfile changes
To apply the modifications made in =~/.spguimacs= press ~SPC f e R~. It will
...
=~/.spguimacs= looks correct. This will check, among other things, whether the
...
Five special functions in the =~/.spguimacs= file can be used to perform
...
at the end of your =~/.spguimacs= file in the function
...
=dotspacemacs-configuration-layers= variable of your =~/.spguimacs=.
...
=dotspacemacs-configuration-layer-path= in your =~/.spguimacs=. For instance
...
the file =~/.spguimacs.env=. This file is automatically created for you by
...
creates the =~/.spguimacs.env= file. These ignored variables are listed in
...
with ~SPC f e C-e~. Note that this action will overwrite =~/.spguimacs.env=.
...
=~/.spguimacs.env=. In all cases ~SPC f e E~ calls the function
...
see the file =~/.emacs.d/core/template/.spguimacs.template= to copy it.
...
can be bound in your =~/.spguimacs= file or in individual layers.
...
in the function =dotspacemacs/user-init= of your =~/.spguimacs=:
...
*** Choosing themes
It is possible to define your default themes in your =~/.spguimacs= with the
...
=.spguimacs= file, this setting will be used for the =default= and =fixed-pitch= faces.
...
| ~SPC t n n~ | toggle line numbers as configured in .spguimacs                   |
...
setting the =dotspacemacs-line-numbers= variable in your =~/.spguimacs= to =t=.
...
=dotspacemacs-mode-line-unicode-symbols= to =nil= in your =~/.spguimacs= will
...
This sequence can be customized in your =~/.spguimacs=.
...
with the variable =dotspacemacs-emacs-command-key= of your =~/.spguimacs=.
...
=dotspacemacs-major-mode-leader-key= in your =~/.spguimacs=. For example to
...
=dotspacemacs/user-config= in your =~/.spguimacs= file, to be able to use
...
=dotspacemacs-smooth-scrolling= variable in your =~/.spguimacs= to =nil=:
...
| Key binding   | Description                                                                                             |
|---------------+---------------------------------------------------------------------------------------------------------|
| ~SPC f e d~   | open the spacemacs dotfile (=~/.spguimacs=)                                                             |
| ~SPC f e D~   | open =ediff= buffer of =~/.spguimacs= and =.spguimacs.template=                                         |
| ~SPC f e e~   | open the =~/.spguimacs.env= file where environment variables are set or goes to =dotspacemacs/user-env= |
| ~SPC f e E~   | reload the environment variables by executing the function =dotspacemacs/user-env=                      |
| ~SPC f e C-e~ | reinitialize the =~/.spguimacs.env= file by importing system and shell environment variables            |
...
=dotspacemacs-smartparens-strict-mode= of you =~/.spguimacs=.
...
You can set variable =dotspacemacs-scratch-buffer-persistent= with a non-nil value in your =.spguimacs= file to enable this feature.

**** Unkillable scratch
Make your =*scratch*= buffer not killable (bury instead).
You can set variable =dotspacemacs-scratch-buffer-unkillable= with a non-nil value in your =.spguimacs= file to enable this feature.
...
| ~SPC D f .~   | start an ediff session between your =.spguimacs= and its default template in Spacemacs =core=                                     |
...
variable =dotspacemacs-persistent-server= to =t= in your =~/.spguimacs=.
